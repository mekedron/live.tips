...
- **La cuenta es opcional.** La app funciona sin ninguna cuenta, y eso sigue siendo lo
  ...
  iniciar sesión — y entonces parte de ello se guarda en un servidor. Qué es qué, se explica
  más abajo.
...
- **En la configuración por defecto, la app solo habla con Stripe** — con ningún servidor de
  live.tips.
- El único servidor que ejecutamos es un pequeño relé en Firebase, de Google. Existe si un
  artista activa Revolut, MobilePay o Monzo — o si inicia sesión.
...
- El **historial de propinas, el historial de sesiones, la meta y los ajustes de la app** se
  guardan en el almacenamiento local del dispositivo. Esto incluye los nombres y mensajes que
  los fans adjuntan a sus propinas.
...
informador de fallos, sin notificaciones push y sin código publicitario — ninguno, ni siquiera
desactivados.
...
| --- | --- |
| Tus **bandas** — nombres, ajustes del bote de propinas y de los métodos de pago, texto del cartel, metas | para que una banda exista en todos los dispositivos en los que inicies sesión |
| Tu **clave restringida de Stripe** y el secreto de la página de propinas del relé | en un documento de secretos que solo tu uid puede leer, y en caché en el llavero de cada uno de tus dispositivos |
| Los **ajustes de la app** | para que un dispositivo que añadas ya esté configurado |
| **Registros de sesiones e historial de propinas** — incluidos **los nombres y los mensajes que los fans adjuntan a sus propinas** | porque ese historial es exactamente lo que pediste ver en el otro dispositivo |
...
| Tus **dispositivos** — el nombre que cada uno se da a sí mismo («iPhone de Nikita»), su plataforma y su modelo, cuándo se vio por primera y última vez | para que Ajustes → Seguridad pueda listarlos, y puedas revocar uno |
...
artista, como parte del historial sincronizado de ese mismo artista.** Ninguna otra cuenta
puede leerlos, nosotros no los miramos y de ellos no se deriva nada — pero ahí están, y
conviene que lo sepas antes de iniciar sesión.
...
Firebase Auth, Cloud Firestore y nuestras Cloud Functions se ejecutan en la **Unión Europea** —
la base de datos en la multirregión `eur3` de Google, las funciones en `europe-west1`. Google
...
se rige por esos términos, no por nosotros.
...
tarjeta, y no tenemos acceso a la cuenta de Stripe del artista.

La app del artista lee sus propias propinas de Stripe usando la clave restringida del propio
artista — directamente del dispositivo a `api.stripe.com`. **No hay ningún servidor de live.tips
en ese camino, y nunca lo hubo.** El nombre y el mensaje de un fan, si dejó alguno, viajan de
Stripe al dispositivo del artista y ahí se detienen — a menos que el artista haya iniciado
sesión, en cuyo caso el dispositivo también los guarda en el historial de Firestore de ese
artista, como se ha explicado más arriba.
...
usuario de Revolut, su Box ID de MobilePay, su usuario de Monzo). Todo ello es información que
el artista está publicando deliberadamente para los fans de todos modos.
...
Ese es todo el formulario. Sin correo, sin teléfono, sin cuenta.

- La propina se escribe en una **cola de entrega** — un único documento que existe para ser
  entregado a la pantalla del artista. Cuando la pantalla muestra la propina, **el dispositivo
  del artista borra ese documento.** El borrado *es* el acuse de recibo; no hay ninguna marca
  de «entregada», porque no queda ningún registro que marcar.
- Si la pantalla del artista está desconectada — móvil bloqueado, sin cobertura — la propina
  **espera en esa cola hasta una hora**, para que no se pierda sin más, y se entrega en cuanto
  la pantalla se reconecta. Si nadie se reconecta, se **borra sin haber sido vista**, barrida
  por una tarea programada, viniera o no alguien a buscarla.
- **Esa cola es el único lugar donde se llega a guardar texto escrito por un fan en nuestro
  servidor, y una hora es su límite absoluto.** Si el artista tiene la sesión iniciada, su
  dispositivo conserva después la propina en *su* historial de Firestore — porque ese es su
  historial, y es justo para lo que inició sesión.
...
- El relé no guarda **ningún historial de propinas**. No puede mostrarte a ti, ni a nosotros, ni
  a nadie una lista de quién dio propina a quién.
...
| --- | --- | --- |
| **Google (Firebase)** | Las cuentas, los datos sincronizados de un artista con sesión iniciada, el relé, los registros del servidor | La cuenta opcional y el relé opcional |
| **Stripe** | Los datos de pago del fan, como responsable independiente | Las propinas con tarjeta |
...
- Gestionar una cuenta que has pedido, sincronizar tus propios datos en tus propios
  dispositivos, ejecutar el relé para un artista que lo activó y entregar la propina de un fan
  a la pantalla a la que iba dirigida: **ejecución de un servicio que has solicitado**.
...
- **Tu cuenta entera, con un solo toque: la app todavía no tiene ese botón.** Preferimos
  ...
  **[contact@live.tips](mailto:contact@live.tips)** y borraremos la cuenta y todo lo que
  ...
  sustancial y deja atrás una cuenta vacía.
...
o una banda desde la app al instante, las propinas de los fans no entregadas se evaporan en una
hora y, si nunca inicias sesión, nada de ello estuvo jamás en otro sitio que tu propio
dispositivo.
