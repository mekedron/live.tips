...
export async function readJsonBody(request: Request): Promise<Result<Record<string, unknown>>> {
  const raw = await request.arrayBuffer();
  if (raw.byteLength > MAX_BODY_BYTES) return err(413, "request body too large");
  try {
...
// ---------------------------------------------------------------------------
// Tip validation (structure only — the jar's DO checks method availability
...
/** jarId as it appears in URLs — reject junk before it ever touches a DO. */
