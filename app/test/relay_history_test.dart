...
void main() {
  ...
  test('round trip: appended tips come back newest first with fields intact',
    ...
    await store.appendRelayHistory([relayTip(0), relayTip(1)]);
    await store.appendRelayHistory([relayTip(2)]);

    final history = store.readRelayHistory();
  ...
  test('duplicate ids are dropped — within a batch and against the store',
      () async {
    final store = await freshStore();
    await store.appendRelayHistory([relayTip(0)]);
    // Relay redelivery / resumed-session replay: same tip, same id.
    await store.appendRelayHistory([relayTip(0), relayTip(0), relayTip(1)]);
  ...
  test('the archive is capped — oldest beyond ${LocalStore.relayHistoryCap} '
    ...
    await store.appendRelayHistory(batch);
